"""Create a small synthetic SurgWMBench dataset for tests and smoke checks."""
    ...
    *,
    ...
    _write_json(
        root / "metadata" / "interpolation_config.json",
        {
            ...
            "interpolation_methods": list(INTERPOLATION_METHODS),
            "interpolated_confidence": 0.6,
            "interpolated_label_weight": 0.5,
            "clip_coords": False,
        },
    )
    _write_json(root / "metadata" / "validation_report.json", {"error_count": 0, "warning_count": 0})
    (root / "README.md").write_text(
        "# SurgWMBench Toy Dataset\n\n"
        "- dataset_version: SurgWMBench\n"
        "- Exactly 20 human anchors per clip.\n"
        "- Dense coordinates are synthetic pseudo coordinates.\n",
        encoding="utf-8",
    )
    ...
    _write_json(
        root / "metadata" / "source_videos.json",
        {
            ...
            "number_of_source_videos": len(source_videos),
