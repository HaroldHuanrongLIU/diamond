...
from diamond_surgwmbench.data.surgwmbench import (
    DATASET_VERSION,
    _coord_from_item,
    _frame_local_index,
    _frame_path_value,
...
def validate_surgwmbench(
    ...
    for entry_idx, entry in enumerate(entries):
        ...
        if not isinstance(annotation, dict):
            ...
            continue
        if annotation.get("dataset_version") != DATASET_VERSION:
            _add_error(errors, f"{annotation_path}: dataset_version is not {DATASET_VERSION!r}")
        if annotation.get("difficulty") != entry.get("difficulty"):
            _add_error(errors, f"{annotation_path}: difficulty differs from manifest")
        ...
        elif sampled_indices != sorted(sampled_indices) or len(set(sampled_indices)) != len(sampled_indices):
            _add_error(errors, f"{annotation_path}: sampled_indices must be sorted and unique")
