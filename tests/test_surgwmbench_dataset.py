...
def test_sparse_dataset_loads_exactly_20_anchors(toy_root: Path) -> None:
...
def test_dense_dataset_loads_variable_length_coordinates_and_sources(toy_root: Path) -> None:
    dataset = SurgWMBenchClipDataset(
        dataset_root=toy_root,
        manifest="manifests/train.jsonl",
        image_size=32,
...
def test_transition_pair_dataset_returns_sparse_anchor_pairs(toy_root: Path) -> None:
    dataset = SurgWMBenchClipDataset(
        dataset_root=toy_root,
        manifest="manifests/train.jsonl",
        image_size=16,
    ...
    assert len(dataset) == 2 * 19
    assert sample["frames"].shape == (2, 3, 16, 16)
    assert sample["frame_indices"].tolist() == sample["sampled_indices"][:2].tolist()
    assert sample["selected_coords_norm"].shape == (2, 2)
    assert sample["sample_kind"] == "transition_pair_sparse"


def test_dense_transition_pairs_use_frame_to_frame_when_enabled(toy_root: Path) -> None:
    dataset = SurgWMBenchClipDataset(
        dataset_root=toy_root,
        manifest="manifests/train.jsonl",
        image_size=16,
    ...
    assert len(dataset) == (25 - 1) + (31 - 1)
    assert sample["frame_indices"].tolist() == [1, 2]
    assert sample["selected_coord_sources"].shape == (2,)


def test_window_dataset_uses_deterministic_non_overlapping_windows(toy_root: Path) -> None:
    dataset = SurgWMBenchClipDataset(
        dataset_root=toy_root,
        manifest="manifests/train.jsonl",
        image_size=16,
    ...
    assert sample0["frames"].shape == (10, 3, 16, 16)
    assert sample0["frame_indices"].tolist() == list(range(10))
    assert sample1["frame_indices"].tolist() == list(range(10, 20))
    assert sample0["selected_coords_norm"].shape == (10, 2)


def test_interpolation_method_switching_loads_selected_file(toy_root: Path) -> None:
...
def test_private_use_path_alias_resolves_star_directories(tmp_path: Path) -> None:
...
def test_strict_loader_rejects_missing_interpolation_file(tmp_path: Path) -> None:
...
def test_loader_rejects_wrong_dataset_version_unless_allowed(tmp_path: Path) -> None:
...
def test_dataset_loader_does_not_create_random_splits(toy_root: Path) -> None:
    manifests_dir = toy_root / "manifests"
...
def test_frame_dataset_returns_image_and_metadata(toy_root: Path) -> None:
    dataset = SurgWMBenchFrameDataset(toy_root, "manifests/train.jsonl", image_size=24)
...
def test_raw_video_dataset_supports_extracted_frame_backend(toy_root: Path) -> None:
