"""SurgWMBench clip and frame datasets.
...
FrameSampling = Literal["sparse_anchors", "dense", "transition_pairs", "window", "all"]
...
# Some transferred archives encode literal "*" path components as this
# private-use character in JSON. Keep the JSON untouched and only use this as a
# filesystem fallback when the canonical path does not exist.
...
def read_jsonl_manifest(path: str | Path) -> list[dict[str, Any]]:
    """Read a SurgWMBench JSONL manifest."""
...
class SurgWMBenchClipDataset(Dataset):
    """Load SurgWMBench trajectory clips from official final-layout manifests."""
    ...
    def __init__(
        ...
        self.frame_sampling = "dense" if frame_sampling == "all" else frame_sampling
        ...
        self._interpolation_cache: dict[Path, dict[str, Any]] = {}

        if self.frame_sampling not in ("sparse_anchors", "dense", "transition_pairs", "window"):
        ...
        if self.frame_sampling == "window" and max_frames is not None and max_frames <= 0:
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        spec = self._samples[index]
        entry = self.entries[spec.entry_idx]
        if self.strict:
            self._validate_manifest_entry(entry, spec.entry_idx)
        ...
        selected_indices = self._select_indices(num_frames, anchor_local_indices, spec)
        ...
        dense = None
        should_load_dense = self.use_dense_pseudo or self.frame_sampling in ("dense", "window")
        if should_load_dense:
        ...
        selected = self._selected_coordinate_tensors(
        ...
        return {
            ...
            "selected_coords_norm": selected["coords_norm"],
            "selected_coords_px": selected["coords_px"],
            "selected_coord_sources": selected["sources"],
            "selected_label_weights": selected["label_weights"],
            "selected_confidence": selected["confidence"],
            "interpolation_method": method,
            "annotation_path": str(annotation_path),
            "interpolation_path": str(interpolation_path),
            "sample_kind": spec.kind,
            "manifest_index": spec.entry_idx,
        }

    def _build_sample_index(self) -> list[_SampleSpec]:
        samples: list[_SampleSpec] = []
        for entry_idx, entry in enumerate(self.entries):
            num_frames = int(entry["num_frames"])
            sampled = [int(value) for value in entry["sampled_indices"]]
            if self.frame_sampling == "transition_pairs":
                if self.use_dense_pseudo:
                    for start in range(max(num_frames - 1, 0)):
                        samples.append(_SampleSpec(entry_idx, (start, start + 1), "transition_pair_dense"))
                else:
                    for start, stop in zip(sampled[:-1], sampled[1:]):
                        samples.append(_SampleSpec(entry_idx, (start, stop), "transition_pair_sparse"))
            elif self.frame_sampling == "window":
                if self.max_frames is None:
                    samples.append(_SampleSpec(entry_idx, tuple(range(num_frames)), "window"))
                else:
                    for start in range(0, num_frames, self.max_frames):
                        stop = min(start + self.max_frames, num_frames)
                        samples.append(_SampleSpec(entry_idx, tuple(range(start, stop)), "window"))
            else:
                samples.append(_SampleSpec(entry_idx, (), self.frame_sampling))
        if not samples:
            raise ValueError(f"No samples were indexed from manifest: {self.manifest_path}")
        return samples
    ...
    def _validate_manifest_entry(self, entry: dict[str, Any], index: int) -> None:
        ...
        sampled_int = [int(value) for value in sampled]
        if sampled_int != sorted(sampled_int) or len(set(sampled_int)) != len(sampled_int):
            raise ValueError(f"Manifest entry {index} sampled_indices must be sorted and unique.")
        if sampled_int and (sampled_int[0] < 0 or sampled_int[-1] >= int(entry["num_frames"])):
            raise ValueError(f"Manifest entry {index} sampled_indices contain values outside clip bounds.")
        difficulty = entry.get("difficulty")
        if difficulty not in ("low", "medium", "high", None):
            raise ValueError(f"Manifest entry {index} difficulty must be low, medium, high, or null.")

    def _validate_annotation(self, entry: dict[str, Any], annotation: dict[str, Any], annotation_path: Path) -> None:
        self._check_version(annotation.get("dataset_version"), str(annotation_path))
        required = (
            "dataset_version",
            "frames",
            "human_anchors",
        ...
        if missing:
            raise ValueError(f"{annotation_path}: annotation is missing required fields: {missing}")
        ...
        if sampled != [int(value) for value in entry.get("sampled_indices", [])]:
            raise ValueError(f"{annotation_path}: sampled_indices differ from manifest")
    ...
    def _load_dense_coordinates(
        ...
        if self.cache_annotations and interpolation_path in self._interpolation_cache:
            return self._interpolation_cache[interpolation_path]
        ...
        if self.strict:
            self._check_version(interpolation.get("dataset_version"), str(interpolation_path))
        ...
        if self.cache_annotations:
            self._interpolation_cache[interpolation_path] = dense
        return dense
    ...
    def _select_indices(self, num_frames: int, anchor_indices: list[int], spec: _SampleSpec) -> list[int]:
        if spec.selected_indices:
            return [int(index) for index in spec.selected_indices]
        if self.frame_sampling == "sparse_anchors":
            return [int(index) for index in anchor_indices]
        if self.frame_sampling == "dense":
    ...
    def _selected_coordinate_tensors(
        self,
        *,
        ...
        dense: dict[str, torch.Tensor] | None,
    ) -> dict[str, torch.Tensor]:
        anchor_pos_by_local = {int(local_idx): pos for pos, local_idx in enumerate(anchor_local_indices)}
        if dense is None:
            missing = [index for index in selected_indices if index not in anchor_pos_by_local]
            if missing:
                raise ValueError(
                    "Selected non-anchor frames require dense interpolation coordinates. "
                    f"First missing local_frame_idx={missing[0]}"
                )
            positions = torch.as_tensor([anchor_pos_by_local[index] for index in selected_indices], dtype=torch.long)
            count = len(selected_indices)
            return {
                "coords_px": anchor_coords_px[positions],
                "coords_norm": anchor_coords_norm[positions],
                "sources": torch.full((count,), SOURCE_TO_CODE["human"], dtype=torch.long),
                "label_weights": torch.ones(count, dtype=torch.float32),
                "confidence": torch.ones(count, dtype=torch.float32),
            }

        positions = torch.as_tensor(selected_indices, dtype=torch.long)
        return {
            "coords_px": dense["coords_px"][positions],
            "coords_norm": dense["coords_norm"][positions],
            "sources": dense["sources"][positions],
            "label_weights": dense["label_weights"][positions],
            "confidence": dense["confidence"][positions],
        }


class SurgWMBenchFrameDataset(Dataset):
    """Frame-level dataset for optional autoencoding or image diffusion smoke tests."""
