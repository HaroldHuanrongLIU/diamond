"""Collate functions for SurgWMBench loaders."""
...
def _direction_classes(dxdy: torch.Tensor, stay_threshold: float = 1e-8) -> torch.Tensor:
    ...
    The moving classes are E, NE, N, NW, W, SW, S, SE in screen coordinates
    converted to mathematical orientation.
    """
...
def collate_transition_pairs(batch: list[dict[str, Any]]) -> dict[str, Any]:
    """Collate pair samples for single-step diffusion world-model training."""

    if not batch:
        raise ValueError("Cannot collate an empty batch.")
    if any(item["frames"] is None for item in batch):
        raise ValueError("collate_transition_pairs requires return_images=True samples.")
    if any(int(item["frames"].shape[0]) != 2 for item in batch):
        raise ValueError("collate_transition_pairs expects samples with exactly two selected frames.")

    frames = torch.stack([item["frames"] for item in batch], dim=0)
    coords_norm = torch.stack([item["selected_coords_norm"] for item in batch], dim=0)
    frame_indices = torch.stack([item["frame_indices"] for item in batch], dim=0)
    num_frames = torch.as_tensor([int(item["num_frames"]) for item in batch], dtype=torch.long)
    actions_delta, actions_delta_dt, _, _ = _actions_from_coords(coords_norm, frame_indices, num_frames)
    sources = torch.stack([item["selected_coord_sources"] for item in batch], dim=0)
    label_weights = torch.stack([item["selected_label_weights"] for item in batch], dim=0)

    return {
        "frame_t": frames[:, 0],
        "frame_tp1": frames[:, 1],
        "coord_t": coords_norm[:, 0],
        "coord_tp1": coords_norm[:, 1],
        "coord_px_t": torch.stack([item["selected_coords_px"] for item in batch], dim=0)[:, 0],
        "coord_px_tp1": torch.stack([item["selected_coords_px"] for item in batch], dim=0)[:, 1],
        "action_delta_dt": actions_delta_dt[:, 0],
        "coord_source_t": sources[:, 0],
        "coord_source_tp1": sources[:, 1],
        "label_weight_tp1": label_weights[:, 1],
        "frame_idx_t": frame_indices[:, 0],
        "frame_idx_tp1": frame_indices[:, 1],
        "difficulty": [item["difficulty"] for item in batch],
        "metadata": _metadata(batch),
    }
